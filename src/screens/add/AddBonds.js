...
import bondToFB from './BondToFB'
import upBondToFB from '../update/upBondToFB'
...
export default function ({ navigation, route }) {
	...
	const [pickerCusIT, setPickerCusIT] = useState(2)
	...
	async function submit() {
		...
		if(await checkInput()){
			...
			console.log(typeRecurName)
			if(upDatetoFB === 0){
				await bondToFB(valueE, nameE, typeRecurName, inRateName, cusIR, pickerCusIT, dayT, Number(chooseTime), expT, returnY.toFixed(2), totalReturnV.toFixed(2), now, noteB)	
			}else{
				await upBondToFB(valueE, nameE, typeRecurName, inRateName, cusIR, pickerCusIT, dayT, Number(chooseTime), expT, returnY.toFixed(2), totalReturnV.toFixed(2), upDatetoFB.id, noteB)
			}
			...
			<Layout>
				...
				>
					...
					>
						...
						/>
						<Text style = {{paddingTop: 10 }}>อัตราผลตอบแทน</Text>
								<TextInput
									containerStyle={{ marginTop: 15,}}
									placeholder="0"
									rightContent={
