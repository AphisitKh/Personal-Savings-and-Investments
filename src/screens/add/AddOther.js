...
import otherToFB from './OtherToFB'

import DateTimePickerModal from "react-native-modal-datetime-picker";
import Moment from 'moment';
import upOtherToFB from '../update/upOtherToFB';
...
export default function ({ navigation, route }) {
	...
	const [pickerCusIT, setPickerCusIT] = useState('')
	const [cusITName, setCusITName] = useState('')
	...
	function checkInput(){
		...
		if(valueE === ''){
			Alert.alert('มูลค่า', 'กรุณากรอกมูลค่า')
			return false
		}
		if(Number(valueE)<=0){
			Alert.alert('มูลค่า', 'ต้องมีค่ามากกว่า 0')
			return false
		}
		if(pickerCusIT === ''){
		...
		if(showCustom === true && pickercustomRe === 365){
            if(Number(cusET) > 365){
                Alert.alert('ระยะเวลาที่ได้ผลตอบแทน', 'จำนวนที่เป็นไปได้มากที่สุดคือ 365 \nกรุณากรอกใหม่')
				return false
            }else if(Number(cusET) < 1){
                Alert.alert('ระยะเวลาที่ได้ผลตอบแทน', 'จำนวนที่เป็นไปได้น้อยที่สุดคือ 1 \nกรุณากรอกใหม่')
				return false
            }else{    
		    	return true
            }
		...
		if(showCustom === true && pickercustomRe === 12){
            if(Number(cusET) > 12){
                Alert.alert('ระยะเวลาที่ได้ผลตอบแทน', 'จำนวนที่เป็นไปได้มากที่สุดคือ 12 \nกรุณากรอกใหม่')
				return false
            }else if(Number(cusET) < 1){
                Alert.alert('ระยะเวลาที่ได้ผลตอบแทน', 'จำนวนที่เป็นไปได้น้อยที่สุดคือ 1 \nกรุณากรอกใหม่')
				return false
	...
	async function submit() {
		if(await checkInput()){
			...
			let totalReturnV = await assetReturn(pickerCusIT, cusIR, expT, valueE)
			let returnY = Number(totalReturnV) - Number(valueE)
			console.log(totalReturnV, returnY) 
			console.log(inRateName);
			if(upDatetoFB === 0) {
				await otherToFB(valueE, nameE, typeRecurName, inRateName, cusIR, pickerCusIT, dayT, Number(chooseTime), expT, returnY.toFixed(2), totalReturnV.toFixed(2), now, noteB)
			
			}else{
				await upOtherToFB(valueE, nameE, typeRecurName, inRateName, cusIR, pickerCusIT, dayT, Number(chooseTime), expT, returnY.toFixed(2), totalReturnV.toFixed(2), now, noteB)
			...
			<Layout>
				...
				>
					...
					>
						...
						<Text style = {{paddingTop: 10 }}>มูลค่า</Text>
						<TextInput
							containerStyle={{ marginTop: 15 }}
							placeholder="0"
                            rightContent={
						...
						<View style={{flexDirection:'row',width:'100%',}}>
                            <View style={{direction:'ltr'}}>
								<TextInput
									containerStyle={{ marginTop: 15, width: 100, marginEnd: 20 }}
									placeholder="0"
									value={cusIR}
									autoCapitalize="none"
									autoCompleteType="off"
                            ...
                            <View style={{direction:'rtl',marginEnd: 20 ,marginTop: 15}}>
                                <Picker
							        items = {customInType}
							        value = {pickerCusIT}
							        placeholder = "เลือกรูปแบบ"
							        onValueChange = {(val) => pCusITName(val)}
						        />
                            </View>
						</View>
